...
public static  class GameObjectExtensions 
{
    ...
    /// <typeparam name="T"></typeparam>
    /// <returns></returns>
    public static T InstantiateDisabled<T>(T originalObject, Transform parent = null, bool worldPositionStays = false) where T : Object
    {
        if (!GetActiveState(originalObject))
        {
    ...
    /// <typeparam name="T"></typeparam>
    /// <returns></returns>
    public static T InstantiateDisabled<T>(T originalObject, Vector3 position, Quaternion rotation, Transform parent = null) where T : Object
    {
        if (!GetActiveState(originalObject))
        {
    ...
    private static bool GetActiveState<T>(T targetObject) where T : Object
    {
        switch (targetObject)
        {
    ...
    private static void SetActiveState<T>(T targetObject, bool newState) where T : Object
    {
        switch (targetObject)
        {
            case GameObject gameObject:
    ...
    private static void SetParent<T>(T targetObject, Transform parent, bool worldPositionStays) where T : Object
    {
        switch (targetObject)
        {
            case GameObject gameObject:
