...
public struct MMStateChangeEvent<T> where T : struct, IComparable, IConvertible, IFormattable
{
    ...
    public CustomStateMachine<T> TargetStateMachine;
    public T NewState;
    public T PreviousState;

    public MMStateChangeEvent(CustomStateMachine<T> stateMachine)
    {
...
/// </summary>
public class CustomStateMachine<T> : MonoBehaviour, MMIStateMachine where T : struct, IComparable, IConvertible, IFormattable
{
    ...
    public CustomStateMachine(GameObject target, bool triggerEvents)
    {
        this.Target = target;
