...
public class ButtonActivated : MonoBehaviour
{
    ...
    [InspectorGroup("Requirements", true, 10)]
    [Information("Here you can specify what is needed for something to interact with this zone. Does it require the ButtonActivation character ability? Can it only be interacted with by the Player? ", InformationAttribute.InformationType.Info, false)]
    ...
    [InspectorGroup("Activation Conditions", true, 11)]

    [Information("Here you can specific how that zone is interacted with. You can have it auto activate, activate only when grounded, or prevent its activation altogether.", InformationAttribute.InformationType.Info, false)]
    ...
    [InspectorGroup("Number of Activations", true, 12)]

    [Information("You can decide to have that zone be interactable forever, or just a limited number of times, and can specify a delay between uses (in seconds).", InformationAttribute.InformationType.Info, false)]
    ...
    [InspectorGroup("Animation", true, 14)]
    ...
    [InspectorGroup("Visual Prompt", true, 15)]

    [Information("You can have this zone show a visual prompt to indicate to the player that it's interactable.", InformationAttribute.InformationType.Info, false)]
    ...
    [Condition("UseVisualPrompt", true)]
    ...
    [InspectorGroup("Actions", true, 17)]
    ...
    protected UnityEngine.Coroutine _autoActivationCoroutine;
    ...
    {
        if (!Layers.LayerInLayerMask(collider.layer, TargetLayerMask))
        {
            return false;
        }
